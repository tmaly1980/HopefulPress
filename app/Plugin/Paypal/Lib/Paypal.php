 ...
 **/
	...
	/*
		if($token = Configure::read("Paypal.token")) 
		{
			# TRYT THIS:
			# $this->oAuthClientId = $token['access_token']; $this->oAuthSecret = $token['secret'];
			return $token; # FOR NOW, TESTING...
		}
		*/
 ...
 **/
	...
	public function planAllUrl($page_size=100,$page=0) {
		return $this->getRestEndpoint() . "/v1/payments/billing-plans?page_size=$page_size&page=$page";
	}

	public function createPlanUrl() {
		return $this->getRestEndpoint() . "/v1/payments/billing-plans";
	}

	public function activatePlanUrl($plan_id) {
		return $this->getRestEndpoint() . "/v1/payments/billing-plans/$plan_id";
	}

	public function billingAgreementUrl() {
		return $this->getRestEndpoint() . "/v1/payments/billing-agreements";
	}
...
################## NEW STUFF FOR RECURRING PAYMENTS.....
	public function planAll()
	{
		...
		do
		{
			$this->HttpSocket->reset();
			// Get endpoint
			$endPoint = $this->planAllUrl(100,$page); # Current page.
			...
			# Append to list.
			$total_pages = $responseArray['total_pages'];
	...
	public function planCreate($planDetails)
	{
		// HttpSocket
		if (!$this->HttpSocket) {
			$this->HttpSocket = new HttpSocket();
		...
		#################################
		# Create plan
		$endPoint = $this->createPlanUrl();
		// JSON encode our card array
		$json = json_encode($planDetails);#$this->formatStoreCreditCardArgs($planDetails));
		# SCREW validation....
	...
	public function updatePlan($plan_id,$planDetails)
	{
		// HttpSocket
		if (!$this->HttpSocket) {
			$this->HttpSocket = new HttpSocket();
		...
		#################################
		# Create plan
		$endPoint = $this->activatePlanUrl($plan_id);
		// JSON encode our card array
		$json = json_encode($planDetails);#$this->formatStoreCreditCardArgs($planDetails));
		# SCREW validation....
	...
	public function createBillingAgreement($billingDetails) # Recurring charge.
	{
		// HttpSocket
		if (!$this->HttpSocket) {
			$this->HttpSocket = new HttpSocket();
		...
		#################################
		# Create plan
		$endPoint = $this->billingAgreementUrl();
		// JSON encode our card array
		$json = json_encode($billingDetails);#$this->formatStoreCreditCardArgs($planDetails));
		# SCREW validation....
