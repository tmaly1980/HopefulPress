...
<? $id = !empty($this->request->data["VolunteerPage"]["id"]) ? $this->request->data["VolunteerPage"]["id"] : ""; ?>
<? $this->assign("page_title", $id ? "Edit Volunteer Page" : "Add Volunteer Page"); ?>
...
<? } else { ?>
	<?= $this->Html->blink("back", "All Volunteer Pages", array("action"=>"index")); ?>
<? } ?>
<? $this->end(); ?>

<div class="pages form">

			<?php echo $this->Form->create('VolunteerPage', array('role' => 'form')); ?>
				...
				<? if(!empty($id)) { ?>
					<?= $this->Html->delete("Delete Page", array("action"=>"delete",$this->data["VolunteerPage"]["id"]), array("confirm"=>"Are you sure you want to delete this page?")); ?>
				<? } ?>
				</td><td align="right">
