...
<? $id = !empty($this->request->data["AdoptionPage"]["id"]) ? $this->request->data["AdoptionPage"]["id"] : ""; ?>
<? $this->assign("page_title", $id ? "Edit Adoption Page" : "Add Adoption Page"); ?>
...
<? } else { ?>
	<?= $this->Html->blink("back", "All Adoption Pages", array("action"=>"index")); ?>
<? } ?>
<? $this->end(); ?>

<div class="pages form">

			<?php echo $this->Form->create('AdoptionPage', array('role' => 'form')); ?>
				...
				<? if(!empty($id)) { ?>
					<?= $this->Html->delete("Delete Page", array("action"=>"delete",$this->data["AdoptionPage"]["id"]), array("confirm"=>"Are you sure you want to delete this page?")); ?>
				<? } ?>
				</td><td align="right">
