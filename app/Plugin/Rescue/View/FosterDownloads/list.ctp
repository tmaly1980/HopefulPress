...
foreach($downloads as $download) { 
	if(!empty($download["FosterDownload"])) { $download = $download['FosterDownload']; } 
	$mime = $download['type'];
	...
	<div id="Download_<?= $download['id'] ?>" class="Download item mime <?= $mimeClass ?> <?= $mimeGroup ?>">
		<div>
			<?= $this->Html->link($download['title'], "/rescue/foster_downloads/download/{$download['id']}/{$download['name']}", array('class'=>'')); ?>
			<? if($this->Html->can_edit($download)) { #!empty($in_admin) && ($this->Admin->access($download))) { ?>
			<span class="marginleft25 controls">
				<?= $this->Html->edit('', array('user'=>1,'controller'=>'foster_downloads','action'=>'edit',$download['id']), array('class'=>'btn-primary btn-xs')); ?>
				&nbsp;
				<?= $this->Html->delete('', array('user'=>1,'controller'=>'foster_downloads','action'=>'delete',$download['id']), array('class'=>'btn-danger btn-xs','confirm'=>'Are you sure you want to remove this download?')); ?>
			</span>
			<? } ?>
			<div class='clear'></div>
