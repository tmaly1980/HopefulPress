...
<div id='Faqs'>
	<? foreach($faqs as $faq) { 
		if(!empty($faq["VolunteerPageFaq"])) { $faq = $faq['VolunteerPageFaq']; } ?>
		<div id="Faq_<?= $faq['id'] ?>" class="Faq item block margintop20">
			<div>
				...
				<span class='paddingleft10'>
					<?= $this->Html->edit(null, array('admin'=>1,'controller'=>'volunteer_faqs','action'=>'edit',$faq['id']), array('class'=>'btn-xs btn-primary','title'=>'Edit/change this question')); ?>
					&nbsp;
					<?= $this->Html->delete(null, array('admin'=>1,'controller'=>'volunteer_faqs','action'=>'delete',$faq['id']), array('class'=>'btn-xs btn-danger','title'=>'Remove this question','confirm'=>'Are you sure you want to remove this question?')); ?>
				</span>
				<? } ?>
				</h4>
				<div class='clear'></div>
