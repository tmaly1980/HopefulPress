...
foreach($downloads as $download) { 
	if(!empty($download["VolunteerDownload"])) { $download = $download['VolunteerDownload']; } 
	$mime = $download['type'];
	...
	<div id="Download_<?= $download['id'] ?>" class="Download item mime <?= $mimeClass ?> <?= $mimeGroup ?>">
		<div>
			<?= $this->Html->link($download['title'], array('controller'=>'volunteer_downloads','action'=>'download',$download['id'], $download['name']), array('class'=>'')); ?>
			<? if($this->Html->can_edit($download)) { #!empty($in_admin) && ($this->Admin->access($download))) { ?>
			<span class="marginleft25 controls">
				<?= $this->Html->edit('', array('admin'=>1,'controller'=>'volunteer_downloads','action'=>'edit',$download['id']), array('class'=>'btn-primary btn-xs')); ?>
				&nbsp;
				<?= $this->Html->delete('', array('admin'=>1,'controller'=>'volunteer_downloads','action'=>'delete',$download['id']), array('class'=>'btn-danger btn-xs','confirm'=>'Are you sure you want to remove this download?')); ?>
			</span>
			<? } ?>
			<div class='clear'></div>
